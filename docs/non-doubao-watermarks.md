...
- **建档案（每工具一次）**：
  ...
  - **文件夹自动聚类建档（推荐入口）**：`learn-mine <dir|images...> [--label-prefix <名>] [--min-samples N] [--corner-tol F] [--recursive] [--dry-run] [--out DIR]`。**不必手工分组**：逐图检测 → 按「同尺寸 + 水印框右下角落位 `(x2/w,y2/h)`」聚簇 → 每簇 ≥3 张交 `auto_discover` 学习（多候选框 + `fit` 择优）→ 自检 ≥2/3 才收 → 写复查拼图 `--out/<id>-review.png`。实测在 `dist/`（豆包 4 张 + 千问 3 张 + 零星他尺寸）上一次挖出 `mined-2848x1600`、`mined-1760x2368` 两档（fit 0.301/0.179、自检 4/4 与 3/3），其余尺寸不足 3 张的簇按 `reason` 跳过。**聚类标识只能用落位**：检测框尺寸逐帧漂移大（豆包 853x200/1149x298/1014x227），且两个坐标**要分别除以 w/h**（都除以短边跨长宽比不可比，会把同款水印拆成多簇）。只在右下角找水印，别处配 `--box`。
