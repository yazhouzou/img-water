...
use doubao_clean::{fetch, pipeline, MODEL_URL_DEFAULT};
use serde::Serialize;
...
fn env_status() -> EnvStatus {
    let path = doubao_clean::model_path();
...
fn setup_model(app: AppHandle, storage: State<'_, AppStorage>) -> Result<(), String> {
    ...
    let dest = doubao_clean::model_path();
    if dest.exists() {
...
fn run_pipeline(
    ...
    let model_path = doubao_clean::model_path();
