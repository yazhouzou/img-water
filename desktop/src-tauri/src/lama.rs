...
fn infer_crop(
    ...
    let (bw, bh) = ((bbox.x2 - bbox.x1) as i64, (bbox.y2 - bbox.y1) as i64);

    // 以 bbox 中心扩展 margin，clamp 图界，贴边时向另一侧补偿（对齐 iopaint _crop_box）
    ...
    let (sw, sh) = ((r - l) as u32, (b - t) as u32);
    ...
    // 内容居中贴进 512 窗口，pad 区用反射延续内容
    let dx = ((WIN - rw as i64) / 2).max(0) as u32;
    let dy = ((WIN - rh as i64) / 2).max(0) as u32;
    let mut win_img = image::RgbImage::from_pixel(WINDOW, WINDOW, image::Rgb([0, 0, 0]));
    for y in 0..WIN {
        for x in 0..WIN {
            let (cx2, cy2) = (x as i64 - dx as i64, y as i64 - dy as i64);
            let (sx, sy) = if cx2 >= 0 && cy2 >= 0 && cx2 < rw as i64 && cy2 < rh as i64 {
                (cx2, cy2)
            } else {
                (reflect_coord(cx2, rw as i64), reflect_coord(cy2, rh as i64))
            };
            win_img.put_pixel(x as u32, y as u32, *crop_img.get_pixel(sx as u32, sy as u32));
        }
    }
