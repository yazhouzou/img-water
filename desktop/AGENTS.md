...
- ⚠️ **已知局限（待办 A）**：`locate_ncc` 对**稀疏 α**（覆盖率 <20%）会被大量零像素主导 → 分数被压低。已修一半：`locate_ncc_impl(densify)` + `location_template` 把稀疏 α 二值化后膨胀到 ~25% 覆盖率再定位（学习出的豆包 α 覆盖率 4%：0.29~0.49 → 0.60~0.68，过 0.5 阈值）；**只用于精确模式/学习自检**（`locate_profile_dense`/`match_specific`），自动路径 `match_image` 仍用真实 α（避免影响正常的稠密档案）。
...
- ⚠️ **仍未解决（A 的真正根因）**：`learn-auto/learn-batch` 学出的 α **不够忠实**——自动框（如 853×200）裁出的 α 形状/尺度与真实水印（豆包 307×102）不符，于是定位到了但**放歪**（mask 仅 3896px、模板分只从 58.6 掉到 57.7）→ `verify_paths` 判 FAIL 拒写（安全但不可用）。即定位器已能"找到"，卡在**学习器的框/笔画检测保真度**（风景图上的弱水印难框准）。`learn-pair`（黑底+白底）/`learn-solid` 不受影响。
