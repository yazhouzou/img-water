...
3. v0.3.4 起检测已全图化：`detect_watermark_boxes()`（Rust + Python 同步）扫描全图白字聚类，支持左上/居中/多处分散水印，返回多框并画多矩形遮罩；`lama.rs` 按 mask 连通块多窗口推理（每块 ≤496px）。检测仍限“纯白文字（≥248）+灰描边”特征，非白字水印（深色、彩色、半透明）检测不到，仍需 `--mask-box` 手动指定。
