...
5. **任意位置文字水印（`--any-position`，默认关）**：默认管线只保留贴右下角的检出框（防雪景/busy photo 误检毁图）；显式开启后保留全部文字性通过的框，且启用扩展检测：纯白字（≥248）+ 灰白/半透明字多阈值扫描（230→170，corner 兜底同款经验全图化）+ 深色字负片检测（≤40，亮背景暗字）。能力边界以 `tools/synthetic_watermark_test.py` 合成矩阵为准（分层量化：检测层/管线默认层/--any-position 层，当前 43/75）：纯白字任意位置全通；灰白字暗/渐变背景全通；深色字亮背景全通；**已知不可分边界**：低对比灰字亮背景（220 字 vs 235 墙，亮度阈值物理不可分）、彩色字（未实现）、黑底半透明字（165 灰度低于阈值下限 170）——这些用 `--mask-box` 手动指定。复杂照片背景（雪景类）--any-position 下必误检（11-15 框），开启后必须人工复查候选图再覆盖。`--mask-box` 支持分号分隔多框（`x1,y1,x2,y2;x1,y1,x2,y2`）。App 端（pipeline.rs）未同步 `--any-position`/多框/扩展检测。后续补齐方向：彩色字检测、低对比结构检测（非亮度阈值）、雪景误检防御（行结构约束强化）。
