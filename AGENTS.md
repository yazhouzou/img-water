...
$P $S --inverse inpaint [文件]     # 额外启用 stamp 逆解（逐图标定 gain+墨色 C + 结果择优，可保真实纹理，半透明水印压在花丛/枝叶上时**优于 SD**，故默认关）
...
## 遮罩规则
按当前图尺寸生成，不可假设尺寸相同。
1. **自动检测（默认）**：全图白字 + 右下兜底，框过**字符行判据**，膨胀只可 5x5 一次（阈值/判据参数见 `docs/lessons.md` §10）。右下兜底另有**字符高带**：字符/行块高须落在短边 `2.6%~5.0%`（豆包字形 ≈3.5%），否则已去水印图右下角的背景碎块（地毯/纸面/花丛，1.3%~2.0%）会被聚成"字符行"误修；行块宽高比 `2.5 ≤ 比 ≤ 8`（水印整行 4.0~4.6，**矮胖单块 1.2~1.9** 会被 9x3 膨胀连成"行"，下限挡掉）、x 投影段数 ≥4（**亮沙地/栏杆宽亮带**在 248 阈只切出 3 段，会被当"水印行"，下限挡掉）。
2. **只留贴右下角的框**（豆包必贴右下），分散水印用 `--mask-box`。
3. **检不到 → 跳过**（空 mask 不进模型）；`--mask-box` 指定则强制。
4. **模板 mask（默认亮字 α + 结果驱动升级）**：豆包字形固定，资产 `tools/doubao-wm-template.png`/`.json`；按短边缩放后在右下角 40px 窗口用**顶帽 gap-score** 匹配；命中写 mask 并写 `.tpl`。**命中须 raw gap≥`TEMPLATE_MIN_SCORE`(20) 且顶帽 gap≥`TEMPLATE_TOPHAT_MIN`(15) 双过**——raw 会把"水印处整体偏亮"（强纹理背景亮块，尺度>顶帽核）计入而误检（已去水印 1/6.png raw 27.3/34.6、顶帽仅 ≤6.4；真水印顶帽 ≥19.9）；未过顶帽按未命中处理（上报顶帽分）。mask 来源：**默认模板亮字 α**（`tools/doubao-wm-alpha.png`，只盖笔画、保住水印周围真实纹理，与 Rust/App 一致）> 二值模板回退；**结果驱动升级**——仅当结果模板残留 >`FOOTPRINT_RETRY_MIN`(5) 时改用**含暗描边的完整 stamp footprint**（`tools/doubao-wm-stamp-alpha.png`，8-bit α 与 Rust 同源，α>0.03 + OPEN 3x3）重跑该图。低对比背景（木纹/纸面）亮字 mask 盖不住暗描边会留暗字形残影，footprint 可救；但 footprint 会圈进水印周围高频纹理（花枝/花瓣）被重绘抹平，故**不作默认**（6.png 花枝实测：footprint 抹平、亮字保留）。不足回退整框，再不到则空 mask 跳过。
...
- **逆解默认关（`--inverse` 开）**：逆解按 stamp α/C 逐像素反解，能保真实纹理，但前提是"该图水印与资产 α/C 完全一致"——实拍图上并不总成立，会留**伪影**。**已修根因**：墨色校正项 `kf·ΔC`(kf=α/(1−α)) 在高 α 处可达上百级，`inv − kf·ΔC` 会**越过 0**，旧的 `corr` 漏 clip → 负值在 `astype(uint8)` 上**回绕**（−2→254）成蓝/品红**彩点**（6.png 实测 `[164,157,152]→[20,10,254]`）；这正是 2.png 彩点/4.png 白线的同源根因，clip 回 `[0,255]` 后消失。实测 7 张实拍图中 MAT 版模板残留**在 6 张上更低或相等**，故默认纯 MAT；但**半透明水印压在花丛/枝叶上时逆解保真实结构、显著优于 SD 的生成式糊块**（6.png 花枝被 SD 抹掉、逆解保留）——此类显式 `--inverse`。
...
- **结果驱动 footprint 升级（默认开、1 轮）**：默认用亮字 α mask（保水印周围真实纹理）；若结果模板残留 >`FOOTPRINT_RETRY_MIN`(5)，改用含暗描边的 stamp footprint 重跑（Python `_footprint_retry`、Rust `footprint_retry`），先于过冲/残留重试执行，逆解图跳过。把"花丛保纹理"与"纸面去描边"两全（根因：`6e0823a` 曾把默认一刀切换成 footprint → 6.png 花枝被抹平）。
