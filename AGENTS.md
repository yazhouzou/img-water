...
5. **任意位置文字水印（`--any-position`，默认关）**：默认管线只保留贴右下角的检出框（防雪景/busy photo 误检毁图）；显式开启后保留全部文字性通过的框，且启用扩展检测：纯白字（≥248）+ 灰白/半透明字多阈值扫描（230→160）+ 深色字负片检测（≤40）+ 彩色字色度检测（sat=max-min>60）+ 低对比亮字顶帽检测（31x31 开运算残差≥12）。能力边界以 `tools/synthetic_watermark_test.py` 合成矩阵为准（分层量化：检测层/管线默认层/--any-position 层，当前 57/75）：纯白字任意位置全通；灰白字暗/渐变背景全通；深色字亮背景全通；彩色字纯色/低饱和背景全通（端到端红字 3000→0 验证）；白墙低对比灰字顶帽解法生效。**已知不可分边界（--mask-box 手动兜底）**：①物理零对比（黑底黑字、220 字 vs 220 底）；②真实彩色/纹理照片上的彩色字（背景 20%+ 像素高饱和，全局色度与 sat 顶帽均失效——与 refine 教训同构：无先验局部特征在纹理背景不可靠）；③暗色照片上的半透明/深色字；④黑底半透明字下限 160 仍不可分的更低灰度。雪景类 busy photo --any-position 下必误检（11-16 框），检出>6 框时 prepare 打印复查警告，开启后必须人工复查候选图再覆盖。`--mask-box` 支持分号分隔多框（`x1,y1,x2,y2;x1,y1,x2,y2`）。App 端（pipeline.rs）未同步 `--any-position`/多框/扩展检测。
...
2. **先消除水印信息泄漏，再最小化 mask**："字形复活"有两条路径：(a) mask 外残留的水印边缘像素；(b) mask 内未填充的水印像素被模型上下文感知。粗填（TELEA）先填平 mask 内的水印信息后，(b) 消失，mask 只需盖住 (a)——两个问题不要用一个"大膨胀"去解决。顺序错误（先大 mask 后找膨胀参数）会系统性过拟合到具体图片。
3. **模型能力要匹配场景复杂度**：LaMa 对结构边界（色带/棱线/阴影）重建差，会断裂发暗；MAT 在粗填底色上精修显著更优。均匀背景（黑底/纸面/沙滩）LaMa 够用；复杂结构场景必须 MAT。粗填+MAT 是默认组合。
4. **量化验证优先于目检**：每次改动必须输出与原图的 diff 量化（changed / lost=阴影丢失 / gain=变暗）+ "mask 外零变化"断言（mask 外任何像素被改动都是 bug）。目检拼图会掩盖噪声级差异（LANCZOS 缩小后颗粒残影不可见），量化不会。
5. **区分"被破坏像素"与"生成像素"的极限**：被水印笔画直接压住的像素信息已不可逆，生成式修复是上限；评价标准是宏观结构（色带走向/棱线/光斑位置）与原图一致 + 衔接自然，而非像素级相同。不要追求像素级还原而引入反解/变换等高风险手段（见 α 反解教训）。
