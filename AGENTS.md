...
6. 优先使用 `iopaint` 批量修复；`inpaint` 支持 `--model mat|lama`（默认 **mat**）：MAT（mask-aware transformer）对结构边界重建显著优于 LaMa——光斑锐度、阴影层次、花瓣形态保留更完整（6.png 花墙对比验证），代价推理约慢 12 倍（单图 ~2 分钟 vs ~10 秒）。MAT 模型 `Places_512_FullData_G.pth`（250MB）在 GitHub release 直连被墙，**用 ghfast.top 镜像前缀**手动下载到 `~/.cache/torch/hub/checkpoints/`（iopaint 自动下载会无限挂起）。`inpaint` 用 iopaint 直接推理（**TELEA 粗填已废弃**：iopaint 推理时会把 mask 区 source 像素置零，任何粗填都无法影响模型输入，详见"修复方法论"）。
7. **模板笔画 mask 方案（已集成 `prepare` 管线，复杂场景精确修复）**：豆包水印字形全图固定（"豆包AI生成"半透明白字，α≈0.6 叠加），模板资产 `tools/doubao-wm-template.png` + `.json`（从黑底 2.png 提取：阈值 ≥125 + 3x3 闭运算，填充率仅 ~21%，远小于整框；元数据含参考短边 1600 与模板内笔画 bbox）。`prepare` 三级策略：① **模板优先**——按图片短边比例缩放模板（水印尺寸随短边等比：2848x1600 → 1728x2304 图放大 1728/1600≈1.08，不是固定像素），右下角 40px 窗口内 **顶帽 gap-score** 匹配（先做 31x31 椭圆开运算顶帽扣除局部背景，再 `cv2.matchTemplate` 两次：0/1 模板核取 S_in、全 1 核取窗口和，gap = S_in/N_in − S_out/N_out；顶帽使判据与背景亮度无关——亮背景（纸面/花墙/雪）不再压低"笔画-间隙"绝对差；实测分数 2.png 145 / 5.png 136 / 4.png 96 / 1.png 59 / 6.png 53 / 3.png 33，已去水印图负样本 ≤8.4），阈值 ≥20 命中即写笔画 mask（**连续 α mask，2026-09 起 Python 端**：用黑底 2.png 提取的连续 α 图 `tools/doubao-wm-alpha.png`，mask = α>0.03 的污染像素 + 1px 缓冲，MAT/LaMa 统一一套；α 图完整覆盖抗锯齿带，故 3x3(±1px) 即够、无字形复活，比旧的"二值模板 + 7x7/19x11 大膨胀"少重绘 ~10-30%，显著减少"去掉水印后周边元素被动过"）+ `.tpl` sidecar；② **整框检测回退**（分数不足时走原多阈值+顶帽检测；3.png 纸面低对比水印由顶帽判据命中模板走 ①，不再回退整框抹平折痕）；③ 检测不到 → 空 mask 跳过。`inpaint` 无粗填、直接推理。crop margin 维持默认 128：margin 256 会让 MAT 在"成"字区生成暗色伪影块。
8. **LaMa"见字生字"机制（关键教训）**：笔画级 mask 若字符间隙未被覆盖，间隙里残留的水印字形轮廓（描边/字符边界）会被 LaMa 的 FFT 全局感受野捕捉，模型按"字形延续"填充每个笔画洞 → 文字形状复活（4.png d5 膨胀时"豆"字复活、6.png 3px 膨胀时整行文字回归）。**破解**：水平膨胀到字符间距填满（±9px）使 mask 连片，间隙上下文消失后模型只能用整体纹理填充，文字不再复活且重绘面积远小于整框；垂直方向只需 ±5px 盖住抗锯齿带（19x11 矩形核）——全向 9px 会让 mask 垂直侵入花墙棱线等强结构边界，LaMa 重绘垂直宽带产生混沌（6.png 教训）。**位置对齐比膨胀参数更关键**：mask 手工放置偏 8px 时小膨胀盖不住字形，会误判为"垂直膨胀不足"（6.png 扁核曾因放置在旧位置 (2538,1498) 而残块，正确位置 (2541,1490) 下 ±3px 即可）。**粗填与膨胀的联动**（6.png 定量验证）：粗填（TELEA 插值）已把 mask 区填成背景延续、消除字形亮度信息，因此 **MAT+粗填下 mask 可缩到 ±3px（7x7）**——最小化重绘面积，最大保留字符间隙里的真实画面（修改面积 14506→10707px、阴影丢失 6047→4856、变暗 2693→944，花丛形态与原图高度一致）；±3px 直接 MAT（无粗填）会字形复活，勿去掉粗填。这与 v0.5.3 的"pad 反射镜像水印"是同一类上下文污染坑。**反解路线（2026-09 更新；单层白字模型已证伪，勿重试）**：单层 `bg=(obs−255α)/(1−α)` 失败，根因不是方法而是**水印模型不完整**——实测确认水印是"半透明白字 + 暗色描边"（stamp 逐像素颜色 C：填充区≈255、边缘低至≈64），且混合是 **sRGB 线性**（线性光假设已排除）。正确模型为完整 stamp `obs = α·C + (1−α)·bg`：用多张同款水印、不同背景的参考图联立标定 α 与 C（资产 `tools/doubao-wm-stamp.npz`），再逐像素反解可恢复被覆盖的**真实背景**（非生成）。已集成为**默认开启**的逐图自动择优（`--no-inverse` 可关闭）：gating = 模板命中 + scale≈1.0 + 水印邻域纹理复杂（HF≥9）+ 逐图 α 增益自校正（最小化残影与 α 相关性）+ 饱和像素回退 MAT；6.png 花丛类场景明显优于 MAT，低纹理（暗底/沙面/纸面）与缩放图（4/5，stamp 非简单缩放）仍走 MAT。不可逆地板：obs 饱和像素（6.png 占 mask 12~16%）与高 α 核心的噪声放大 1/(1−α)。**回退策略（2026-09 修正）**：逆解只在【模型失解】时回退 MAT（raw 超出 [0,255]，obs 无法由 α·C+(1−α)·bg 解释）或全通道饱和——旧的"任一通道饱和即回退 MAT"会让 MAT 在亮背景饱和像素上凭空生成深色斑点（6.png 实测原图 239→MAT 37 的黑点 artifact）；改为仅失解回退后消失、模板残留不变。**区分"真实暗斑"与"生成伪影"**：6.png 木纹上一处深斑在原图就存在（(2634,1555) 原图=结果=68），水印移除后因对比变化更显眼，属真实内容非处理引入，不要误当伪影去"修"。**通用原则**：回退/兜底启发式（如"饱和→用另一模型"）本身会注入伪影，宁可只在明确的模型失解条件下回退，也不要用宽泛的启发式触发。
...
.img-inpaint-venv/bin/python tools/remove_doubao_watermark.py inpaint [可选文件列表]  # 默认逐图自动择优：复杂纹理 scale≈1.0 走 stamp 逆解，其余 MAT
.img-inpaint-venv/bin/python tools/remove_doubao_watermark.py --no-inverse inpaint [可选文件列表]  # 强制全部走 MAT
.img-inpaint-venv/bin/python tools/remove_doubao_watermark.py --no-retry inpaint [可选文件列表]  # 关闭残留自动重试，只保留 FAIL 拒绝落盘
.img-inpaint-venv/bin/python tools/remove_doubao_watermark.py overwrite-review [可选文件列表]  # 落盘前过验证闭环，FAIL 拒绝覆盖
.img-inpaint-venv/bin/python tools/remove_doubao_watermark.py --force overwrite-review [可选文件列表]  # 人工确认后强制覆盖
...
## 验证闭环与回归（2026-09 新增，把"人肉发现"换成"机制"）

**结果级验证闭环（`verify_paths` / `verify_repaired`，已接入管线）**：修复后对每张图自动做场景无关的客观自检，`overwrite-review` 落盘前逐图打印 `[PASS]/[WARN]/[FAIL]`，**FAIL 直接拒绝覆盖**（`--force` 可人工强制）；`review-lama`（候选阶段）也会打印。判据：
1. **mask 外零改动**（硬性）：结果与备份原图在 mask 外的任何像素差异 >2 即 FAIL——这是任何水印/任何场景都成立的工程保证，被抓到一定是 bug（曾据此发现逆解越界）。`--inverse` 现已把改写限制在声明 mask 内（stamp α 比模板 mask 宽的那圈不再越界）。
2. **模板残留**（仅模板路径，`.tpl` sidecar 判定）：原图命中豆包模板而修复后仍命中即 FAIL/WARN。非豆包水印不启用此检查（合成文字会碰巧匹配模板造成误报）。
3. **mask 面积占比**：> 8% 告警（过度重绘）。
验证器不依赖 ground truth，故能泛化到没见过的新场景；但它抓不到"本无水印却误修"（检测层问题，见下）。

**自校验 → 自动重处理（默认开，最多 1 轮）**：`inpaint` 在修复（含逆解）后逐图跑上述校验；一旦判定**残留 FAIL**，即把该图 mask 膨胀一级（`RETRY_DILATE = 5x5`，仍紧贴水印、最小侵入）放进隔离子目录**单独重跑**一次模型（避免整批重复推理）并复验——这就是"不完美就重新处理"。只对残留触发（`mask 外改动`属工程 bug，重试无意义）；仍残留则保留 FAIL 交给 `overwrite-review` 拒绝落盘。`--no-retry` 关闭此能力（只留拒绝门）。验证（含自动重试）目前为 Python-only，Rust `pipeline.rs` 未同步。

**回归语料库（`tools/watermark_regression.py`）**：每个新场景修好后固化为永久测试点，改动后量化 PASS/FAIL，**非零退出码可直接接 CI / 提交前预检**：
- `L1`（快，无模型）：真实豆包正样本命中模板、已去水印图不误命中、合成文字水印命中已知 bbox、纯背景不误检；`known` 标记的为已文档化物理边界（低对比/黑底黑字/busy photo 误检），不算失败但会提示 CHANGED。
- `L3`（`--e2e --model lama`，慢）：真实跑 `run` 全链路，再用结果级验证判定（含合成水印"已知 bbox 残留 IoU"泛化检测）。合成背景用【已去水印根目录图的裁切】，禁用带水印的 dist 图/缩放重采样（会引入误检假象）。
- 用法：`.img-inpaint-venv/bin/python tools/watermark_regression.py [--e2e] [--only 组名]`；新场景（如又一张难图）加入 `build_cases` 即成为回归点。
...
2. **先消除水印信息泄漏，再最小化 mask**："字形复活"有两条路径：(a) mask 外残留的水印边缘像素；(b) mask 内未填充的水印像素被模型上下文感知。**注意：(b) 在 iopaint 下不存在——iopaint 推理时会把 mask 区 source 像素置零（MAT/LaMa 受控实验输出逐像素相同，2026-09 确证），粗填/预处理无法影响模型输入，管线里的 TELEA 粗填已作为死代码移除；历史"粗填有效"结论系旧环境误导，勿重试。** mask 只需盖住 (a)。**关键（2026-09 更新）**：用水印连续 α 图（`tools/doubao-wm-alpha.png`）生成 mask = α>0.03 + 1px，α 图已完整覆盖抗锯齿带，字符间隙无残留字形上下文，3x3(±1px) 即够且 MAT/LaMa 均无字形复活；旧二值模板（α>0.5）只盖笔画核心，必须靠 7x7/19x11 大膨胀补抗锯齿，属过度覆盖——正是"去掉水印后周边元素被动过"的根因。
...
4. **量化验证优先于目检**：每次改动必须输出与原图的 diff 量化（changed / lost=阴影丢失 / gain=变暗）+ "mask 外零变化"断言（mask 外任何像素被改动都是 bug）+ **字形复活检测**（核心区/环带内输出比原图亮>40 的白复活 + 输出比原图暗>40 的暗描边复活——量化会漏暗色复活，必须配合 3x 放大目检）。crop margin 64/128 实测持平（crop 缩放非质量瓶颈，128 即可）。这套量化已固化为 `verify_paths`（见"验证闭环与回归"），改动后先跑它、再跑 `tools/watermark_regression.py`。
5. **区分"被破坏像素"与"生成像素"的极限**：被水印笔画直接压住的像素信息已不可逆，生成式修复是上限；评价标准是宏观结构（色带走向/棱线/光斑位置）与原图一致 + 衔接自然，而非像素级相同。**混合边界区（如花丛/亮墙交界被"包"字下半压住）是偏差最大区域**——模型从混合上下文生成，花瓣形态/棱线走向与原图必有形态差，这是信息论极限不是流程缺陷；所有管线参数（膨胀核/margin/粗填/羽化/模型）穷尽实验后仍存在的差异即为此类极限，勿继续投入调参，转向"mask 是否能更精确"的唯一杠杆。像素级还原只能走已集成的 `--inverse`（完整 stamp 反解 + gating + 饱和回退），不要再用单层白字公式或临时反解/变换脚本（见反解教训）。
